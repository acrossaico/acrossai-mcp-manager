...
* **New — a second, plugin-managed MCP server appears on your site (F088).** Updating adds an AcrossAI-branded server at route `acrossai/mcp`, alongside the existing **Default MCP Server**. It arrives **disabled** and advertises nothing until you enable it, so no AI client sees a change you did not ask for. It is plugin-managed: its name, route and description are owned by the plugin, it cannot be deleted from the servers list, and its tools are the AcrossAI toolsets when the AcrossAI Abilities Manager add-on is installed. Your existing server is untouched — same route, same tools, same enabled state.
...
* **Changed — new servers are created as **MCP Adapter**, and the AcrossAI server is no longer singled out.** Creating a server from either the classic form or the Quick Connect wizard now starts it on the MCP Adapter type; pick AcrossAI from the **Server type** dropdown when you want it. The green **Recommended** pill on the plugin-managed AcrossAI server is gone from the servers list, that server's Overview tab and the wizard's server picker, and so is the ordering rule that pinned it to the top — servers now list in the order they were created. Nothing about the server itself changed: it keeps its AcrossAI type, its route and its tools. Which type new servers start on is one declaration, `ServerTypes::seed()`'s `is_default` key, resolved everywhere through the existing `ServerTypes::default_slug()`.
* **Database — schema 1.1.7.** Adds `server_type varchar(32) NOT NULL DEFAULT 'mcp-adapter'` to `{prefix}acrossai_mcp_servers`, applied automatically on the first wp-admin page load after updating. The column default is what backfills existing rows, so every pre-existing server keeps its current behaviour; the plugin-managed AcrossAI server is corrected to the AcrossAI type by the same migration. If you have deliberately changed that server's type, a later re-run will not revert your choice. Schema `1.1.7` also drops a `tools_default_policy` column, which **you will not have** if you are updating from 0.3.3 — it existed only between two development migrations and was never released. The coarse expose/hide tool rule it backed was never honoured at call time, so a server could advertise a tool it then refused; the drop is there to clean up development installs. `abilities_default_policy` on the Abilities tab is a different setting and is unaffected.
