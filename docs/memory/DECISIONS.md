...
- **Reconsider**: If BerlinDB Core adds an auto-diff mode (e.g., an `$auto_diff = true` protected flag that would trigger a Schema-to-live-table comparison), this DEC's manual-callback requirement can relax. Until then, callbacks are mandatory. — **INVOKED 2026-09-24 by F091; see the amendment below.**

**Amendment (2026-09-24, F091) — invoked under this DEC's own Reconsider clause.**

Core did not add an auto-diff mode; this plugin grew one. `Includes\Database\SchemaReconciler` compares each Table's declared Schema against `Schema::from_table()` and materialises missing columns from each Column's own `get_create_string()`. Same relaxation, same reason, so the three-part contract now applies **by kind of change**:

- **Pure column ADDITION** — declare it in `Schema.php` and stop. No `$version` bump, no `$upgrades` entry. The reconciler adds it on the next `admin_init` and at activation, on every install that lacks it. Because the DDL comes from the Column object itself, the physical column can never disagree with the declaration — which a hand-written `ALTER` string always could.
  **Exception**: a column the reconciler will not add still needs the full contract. `SchemaReconciler::is_addable()` is the authority; it excludes `auto_increment` (errno 1075), TEXT/BLOB/JSON/GEOMETRY carrying a default (errno 1101), and the zero date (errno 1067).
- **A new column that also needs a VALUE on existing rows** — declare it, and add a value pass keyed on the reconciler's created-column report. `MCPServer\CreatedColumnBackfill` is the reference implementation. Still no `$version` bump. The pass MUST act only on columns created **in that same request**, per `SeededToolsBackfill`'s doctrine: an operator cannot have expressed a preference about a column that did not exist, so a repair scoped that way can never fight them.
- **REMOVAL, type change, width NARROWING, default change, index change, rename or drop** — the contract is UNCHANGED and MANDATORY. The reconciler is add-only by design, reports these through `acrossai_mcp_schema_drift_detected`, and takes no action. Their callback MUST return `false` when its statement fails, because the unstamped version is the only retry they have. ADD-column callbacks deliberately keep returning `true` — the reconciler is their retry, and a `false` there would stall every later migration behind a possibly-unfixable `ALTER`.

**Reviewer rule, replacing "a `Schema.php` diff must have a paired `Table.php` diff"**: a diff that only ADDS a column array needs no `Table.php` diff. A diff that REMOVES, narrows or retypes an entry MUST have one — and `tests/phpunit/Database/SchemaManifestTest.php` fails the build if it does not, so this is no longer a review-memory obligation. Note the grep gate B34 asked for is not implementable: "was this narrowed" is a diff predicate, and `verify-f021-gates.sh` runs whole-repo greps against a `fetch-depth: 1` checkout with no merge base.

**Second defect this closes.** D28 as written addressed only "version bumped without a callback". It did not cover the *phantom version stamp*: `Table::get_pending_upgrades()` returns empty whenever `$this->db_version` is empty, and `upgrade()` then calls `set_db_version()` and returns `true` having run **zero** callbacks. This fires on any site whose table pre-dated the BerlinDB-era option key — every install from the pre-F011 `dbDelta` installer keyed on `acrossai_mcp_manager_db_version`. The damage is a contiguous PREFIX: the stamp takes the value the landed-on release declares, and later migrations then run normally from that false baseline, which is why a production site was observed missing the 1.1.1 and 1.1.2 columns while holding the 1.1.5 and 1.1.6 ones. No version bump can ever reach such a site; the reconciler is the only mechanism that does.

**Reconsider (revised)**: if `SchemaReconciler` is ever extended to MODIFY or DROP, this split collapses and everything returns to the three-part contract — the licence for the relaxation is precisely that adding a column cannot lose data while narrowing or dropping one can. `bin/verify-f021-gates.sh` gate `F091-1` enforces that property mechanically.
